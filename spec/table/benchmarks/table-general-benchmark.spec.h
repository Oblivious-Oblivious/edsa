#ifndef __EDSA_TABLE_GENERAL_BENCHMARK_SPEC_H_
#define __EDSA_TABLE_GENERAL_BENCHMARK_SPEC_H_
...
static void benchmark_insertion(EdsaTable *table, char **keys, size_t count) {
...
static void benchmark_lookup(EdsaTable *table, char **keys, size_t count) {
  double start_time = get_time();
  size_t not_found  = 0;
  for(size_t i = 0; i < count; i++) {
...
static void benchmark_deletion(EdsaTable *table, char **keys, size_t count) {
  double start_time = get_time();
  for(size_t i = 0; i < count; i++) {
...
static void benchmark_remove_nonexistent(EdsaTable *table, size_t count) {
  double start_time = get_time();
  for(size_t i = 0; i < count; i++) {
    char key[50];
...
static void
benchmark_insert_duplicates(EdsaTable *table, char **keys, size_t count) {
  for(size_t i = 0; i < count; i++) {
    table_add(table, keys[i], i);
  }
