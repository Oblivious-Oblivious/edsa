...
#include <stdint.h> /* int8_t, uint32_t, uint64_t */

p_inline uint32_t MURMURHASH3_ROTL32(uint32_t x, int8_t r) {
  return (x << r) | (x >> (32 - r));
}
...
MurmurHash3_x86_128(const void *key, int len, uint32_t seed, void *out) {
  ...
  for(i = -nblocks; i; i++) {
    ...
    k1 *= c1;
    k1 = MURMURHASH3_ROTL32(k1, 15);
    k1 *= c2;
    h1 ^= k1;

    h1 = MURMURHASH3_ROTL32(h1, 19);
    h1 += h2;
    h1 = h1 * 5 + 0x561ccd1b;

    k2 *= c2;
    k2 = MURMURHASH3_ROTL32(k2, 16);
    k2 *= c3;
    h2 ^= k2;

    h2 = MURMURHASH3_ROTL32(h2, 17);
    h2 += h3;
    h2 = h2 * 5 + 0x0bcaa747;

    k3 *= c3;
    k3 = MURMURHASH3_ROTL32(k3, 17);
    k3 *= c4;
    h3 ^= k3;

    h3 = MURMURHASH3_ROTL32(h3, 15);
    h3 += h4;
    h3 = h3 * 5 + 0x96cd1c35;

    k4 *= c4;
    k4 = MURMURHASH3_ROTL32(k4, 18);
    k4 *= c1;
    h4 ^= k4;

    h4 = MURMURHASH3_ROTL32(h4, 13);
    h4 += h1;
    h4 = h4 * 5 + 0x32ac3b17;
  }
  ...
  h1 = murmurhash3_fmix32(h1);
  h2 = murmurhash3_fmix32(h2);
  h3 = murmurhash3_fmix32(h3);
  h4 = murmurhash3_fmix32(h4);
...
MurmurHash3_x64_128(const void *key, int len, uint32_t seed, void *out) {
  ...
  for(i = 0; i < nblocks; i++) {
    ...
    k1 *= c1;
    k1 = MURMURHASH3_ROTL64(k1, 31);
    k1 *= c2;
    h1 ^= k1;

    h1 = MURMURHASH3_ROTL64(h1, 27);
    h1 += h2;
    h1 = h1 * 5 + 0x52dce729;

    k2 *= c2;
    k2 = MURMURHASH3_ROTL64(k2, 33);
    k2 *= c1;
    h2 ^= k2;

    h2 = MURMURHASH3_ROTL64(h2, 31);
    h2 += h1;
    h2 = h2 * 5 + 0x38495ab5;
  }
